...
/// set message type and flags for a RTM_DELLINK request
pub fn del_link_nl_header(header: &mut NlMsgHeader) {
    const FLAGS: u16 = NLM_F_REQUEST | NLM_F_ACK;
    header.r#type = super::RTM_DELLINK;
    header.flags = FLAGS;
}

#[derive(Debug)]
pub struct DelLinkMsgBuilder<'a, Buffer: std::io::Write> {
    pub buffer: &'a mut Buffer,
    pub nl_msg_header: NlMsgHeader,
    pub if_info_msg: super::IfInfoMsg,
}

impl<'a, Buffer: std::io::Write> MessageBuilder<'a> for DelLinkMsgBuilder<'a, Buffer> {
    type Buffer = Buffer;
    type Input = i32;
    type Output = ();
    type ParseError = ();
    ...
    fn new_with_header(
        ...
        del_link_nl_header(&mut nl_msg_header);

        Self {
            buffer,
            nl_msg_header,
            if_info_msg: super::IfInfoMsg::new(input),
        }
    }
